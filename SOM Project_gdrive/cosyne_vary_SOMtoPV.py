...
wSOMPV0 = 0. * ones((N_SOM, N_PV)) #-0.05 * ones((N_SOM, N_PV))
...
for i in arange(len(stim_strength)):
    for j in arange(len(stim_rf)):
        ...
        ratePy_eq, rateSOM_eq, ratePV_eq = calc_equilibrium(rateIn, bPy, bSOM, bPV,
                            ...
                            wSOMPy0, wSOMSOM, wSOMPV0, wPVPyADD, wPVSOM, wPVPV, 1)
...
## copy py rates
ratePy_eqs_noSOMtoPV = ratePy_eqs
ratePy0_eqs_noSOMtoPV = ratePy0_eqs
...
plot(squeeze(ratePy_eqs[c_idxs[0], :, a_idxs[0]]),
     squeeze(ratePy0_eqs[c_idxs[0], :, a_idxs[0]]), 'ok', ms=5)
plot(squeeze(ratePy_eqs_noSOMtoPV[c_idxs[0], :, a_idxs[0]]),
     squeeze(ratePy0_eqs_noSOMtoPV[c_idxs[0], :, a_idxs[0]]), 'or', ms=5)     
plot([ymin, ymax], [ymin, ymax], c=[0.7, 0.7, 0.7])

axis('scaled')
xlabel('LED Off (spikes/s)')
ylabel('LED On (spikes/s)')
title('Py Firing Rate both')
