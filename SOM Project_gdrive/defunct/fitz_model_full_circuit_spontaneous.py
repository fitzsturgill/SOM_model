...
def calc_equilibrium_x(rateIn, wInPy, wInSOM, wPySOM, wSOMPy, wInPV, wPyPV, wPVPy, wSOMPV=0, wPVSOM=0, SOMbaseline=0, PyBaseline=0, PVbaseline=0, plothist=False):
    """
    Calculates the equilibrium rates of the local bend In -> interneuron system.
    """
    ...
    for i in range(numsteps):
        ...
        ratePy_eq = (dot(rateIn.T, wInPy) + dot(rateSOM.T, wSOMPy) + PyBaseline)  / (dot(ratePV.T, wPVPy) + 1)
        rateSOM_eq = dot(rateIn.T, wInSOM) + dot(ratePy.T, wPySOM) + dot(ratePV.T, wPVSOM) +  SOMbaseline
        
        ratePV_eq = dot(rateIn.T, wInPV) + dot(ratePy.T, wPyPV) + dot(rateSOM.T, wSOMPV) + PVbaseline
...
def calc_equilibrium_0(rateIn, wInPy, wInSOM, wPySOM, wSOMPy, wInPV, wPyPV, wPVPy, wSOMPV=0, wPVSOM=0, SOMbaseline=0, PyBaseline=0, PVbaseline=0, plothist=False):
    """
    Calculates the equilibrium rates of the local bend In -> interneuron system.
    """
...
wPyPV = 10.0 / (N_Py) * ones((N_Py, N_PV))
...
for i in arange(len(touch_strength)):    
    for j in arange(len(touch_angle)):
        ...
        ratePy_eq, rateSOM_eq, ratePV_eq = calc_equilibrium_x(rateIn, wInPy, wInSOM, wPySOM, wSOMPy, wInPV, wPyPV, wPVPy, wSOMPV, wPVSOM, SOMbaseline, PyBaseline, PVbaseline)
        ratePy0_eq, rateSOM0_eq, ratePV0_eq = calc_equilibrium_0(rateIn, wInPy, wInSOM, wPySOM, (wSOMPy), wInPV, wPyPV, (wPVPy), (wSOMPV), wPVSOM, SOMbaseline0, PyBaseline0, PVbaseline0)
