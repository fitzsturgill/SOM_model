...
@author: Paxon   the equations for rateSOM_eq and ratePV_eq are kludgy- we don't have PV --> PV or PV --> SOM inhibitino in our model.
   however, the 2 equations define PV --> PV an PV --> SOM inhibition as divisive, regardless of whether PV --> is subtractive or divisive
   Because wPVPV and wPVSOM are so far set to 0 this doesn't matter, but it is nevertheless formally an issue
"""
...
def calc_equilibrium(rateIn, bPy, bSOM, bPV, wInPy, wInSOM, wInPV, wPyPy, wPySOM, wPyPV, wSOMPy, wSOMSOM, wSOMPV, wPVPy, wPVSOM, wPVPV, additive):
    ...
    for i in range(numsteps):
        ...
        if additive == 1:
            ...
            ratePy_eq = (rateIn * wInPy.T + dot(ratePy.T, wPyPy) + bPy)  / (dot(ratePV.T, wPVPy) + 1) / (dot(rateSOM.T, wSOMPy) + 1)          
...
wPyPV = 0. / N_Py * ones((N_Py, N_PV)) #5. / N_Py * ones((N_Py, N_PV))

wSOMPy = -0.2 * ones((N_SOM, N_Py)) # -2.
...
#SOM divisive
wSOMPyDiv = 0.1 * ones((N_SOM, N_Py)) # -2.
wPySOMDiv = 1. / N_Py * ones((N_Py, N_SOM))
...
ratePy_eqsDiv = zeros((N_Py, len(stim_strength), len(stim_rf)))
rateSOM_eqsDiv = zeros((N_SOM, len(stim_strength), len(stim_rf)))
ratePV_eqsDiv = zeros((N_PV, len(stim_strength), len(stim_rf)))

ratePy0_eqsDiv = zeros((N_Py, len(stim_strength), len(stim_rf)))
rateSOM0_eqsDiv = zeros((N_SOM, len(stim_strength), len(stim_rf)))
ratePV0_eqsDiv = zeros((N_PV, len(stim_strength), len(stim_rf)))
...
# run it with SOM cells divisive
for i in arange(len(stim_strength)):
    for j in arange(len(stim_rf)):
        
        rateIn = stim_strength[i]
        
        ratePy_eqDiv, rateSOM_eqDiv, ratePV_eqDiv = calc_equilibrium(rateIn, bPy, bSOM, bPV,
                            wInPy, wInSOM, wInPV, wPyPy, wPySOMDiv, wPyPV, 
                            wSOMPyDiv, wSOMSOM, wSOMPV, wPVPy, wPVSOM, wPVPV, 2)
                            
        
        ratePy0_eqDiv, rateSOM0_eqDiv, ratePV0_eqDiv = calc_equilibrium(rateIn, bPy, bSOM, bPV,
                            wInPy, wInSOM0, wInPV, wPyPy, wPySOM0, wPyPV, 
                            wSOMPyDiv, wSOMSOM, wSOMPV, wPVPy, wPVSOM, wPVPV, 2)
                            
                        
        ratePy_eqsDiv[:, i, j] = ratePy_eqDiv[:, 0]
        rateSOM_eqsDiv[:, i, j] = rateSOM_eqDiv[:, 0]
        ratePV_eqsDiv[:, i, j] = ratePV_eqDiv[:, 0]
                     
        ratePy0_eqsDiv[:, i, j] = ratePy0_eqDiv[:, 0]
        rateSOM0_eqsDiv[:, i, j] = rateSOM0_eqDiv[:, 0]
        ratePV0_eqsDiv[:, i, j] = ratePV0_eqDiv[:, 0]        
## Save the data
...
plot(squeeze(ratePy_eqsDiv[c_idxs[0], :, a_idxs[0]]),
     squeeze(ratePy0_eqsDiv[c_idxs[0], :, a_idxs[0]]), 'or', ms=5)
...
f4_filename = filename + '-py_pyNAIVE'
