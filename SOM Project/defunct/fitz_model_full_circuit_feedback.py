...
def calc_equilibrium_x(rateIn, wInPy, wInSOM, wPySOM, wSOMPy, wInPV, wPyPV, wPVPy, wSOMPV=0, wPVSOM=0, SOMbaseline=0, wPyPy=0, plothist=False):
    """
    Calculates the equilibrium rates of the local bend In -> interneuron system.
    """
    ...
    for i in range(numsteps):
        ...
        ratePy_eq = (dot(rateIn.T, wInPy) + dot(rateSOM.T, wSOMPy) + dot(ratePy.T, wPyPy))  / (dot(ratePV.T, wPVPy) + 1)
...
def calc_equilibrium_0(rateIn, wInPy, wInSOM, wPySOM, wSOMPy, wInPV, wPyPV, wPVPy, wSOMPV=0, wPVSOM=0, SOMbaseline=0, wPyPy=0, plothist=False):
    """
    Calculates the equilibrium rates of the local bend In -> interneuron system.
    """
...
for i in arange(len(touch_strength)):    
    for j in arange(len(touch_angle)):
        ...
        ratePy_eq, rateSOM_eq, ratePV_eq = calc_equilibrium_x(rateIn, wInPy, wInSOM, wPySOM, wSOMPy, wInPV, wPyPV, wPVPy, wSOMPV, wPVSOM, 20, wPyPy)
        ratePy0_eq, rateSOM0_eq, ratePV0_eq = calc_equilibrium_0(rateIn, wInPy, wInSOM, wPySOM, (wSOMPy), wInPV, wPyPV, (wPVPy), (wSOMPV), wPVSOM, 0, wPyPy)
...
# Inlot parameters
c_idxs = array([12, 18]) # which cells to plot
#c_idxs = array([12])
...
for a in a_idxs:
    for c in c_idxs:
        ...
        col='k'
        plot(squeeze(ratePy_eqs[c, :, a]), squeeze(ratePy0_eqs[c, :, a]), c=col, lw=5)
        plot([0, 70], [0, 70], '--k')
